...
router.get("/service", checkAuthentication, (req, res) => {
  db.Service.findByPk(req.session.customer.id).then((data) => {
    res.json(data);
  });
});
...
router.post("/customer", (req, res) => {
  var { first_name, last_name, email, login_password, phone_number } = req.body;
...
router.post("/login", (req, res) => {
  ...
  db.Customer.findOne({ where: { email: email } })
    ...
    .catch(() => {
      res.send(
        "Username not found. Please return to previous page and try again."
      );
    });
});
...
router.post("/booking", checkAuthentication, (req, res) => {
  const { nick_name, sq_ft, address, city, state, zipcode, price } = req.body;
  ...
  db.Services.create({
    ...
    res.json(Service); //!customer/:id, this id is the customer in the database.
  });
});
